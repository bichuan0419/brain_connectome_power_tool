...
for i = 1:length(sample_size_list)
    parfor j = 1:length(cluster_size_list)
        power_ttest_cluster_size(i,j) = covariate_net_ttest(sample_size_list(i), sample_size_list(i), N, sigma0, ...
    Cohensd, rho_in, rho_out, cluster_size_list(j), Covariance,...
...
% save 
save('ttest_power_vs_SS_cluster_size.mat')
% save figure
saveas(gcf,'ttest_power_vs_SS_cluster_size.eps','epsc');