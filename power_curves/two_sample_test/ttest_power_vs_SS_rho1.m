...
for i = 1:length(sample_size_list)
    parfor j = 1:length(rho_in_list)
        power_ttest_rho_in(i,j) = covariate_net_ttest(sample_size_list(i), sample_size_list(i), N, sigma0, ...
    Cohensd, rho_in_list(j), rho_out, cluster_size, Covariance,...
...
% save 
save('ttest_power_vs_SS_rho1.mat')
% save figure
saveas(gcf,'ttest_power_vs_SS_rho1.eps','epsc');