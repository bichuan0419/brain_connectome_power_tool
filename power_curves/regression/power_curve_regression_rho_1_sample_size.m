...
for i = 1:length(sample_size_list)
    for j = 1:length(rho_in_list)
        power_reg_cluster_size(i,j) = covariate_net_reg(sample_size_list(i), N, sigma0, Cohensf2, rho_in_list(j), rho_out, cluster_size, FWER_threshold, M_rep, M_perm);
    end
end
...
hold off
legend({ ['|\rho_1| = ', num2str(rho_in_list(1))], ...
    ['|\rho_1| = ', num2str(rho_in_list(2))], ['|\rho_1| = ', num2str(rho_in_list(3))], ...
    ['|\rho_1| = ', num2str(rho_in_list(4))], ['|\rho_1| = ', num2str(rho_in_list(5))], ...
    ['|\rho_1| = ', num2str(rho_in_list(6))]}, 'FontSize',16,'Location','southeast')
