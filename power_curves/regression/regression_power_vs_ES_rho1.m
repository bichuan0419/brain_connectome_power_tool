...
for i = 1:length(f2_list)
    parfor j = 1:length(rho_in_list)
        power_reg_rho_in(i,j) = covariate_net_reg(n, N, sigma0,...
    f2_list(i), DF, rho_in_list(j), rho_out, cluster_size, Covariance, ...
...
% save 
save('regression_power_vs_ES_rho1.mat')
% save figure
saveas(gcf,'regression_power_vs_ES_rho1.eps','epsc');